<aside class="col-lg-3 col-md-4 mb-4 mb-md-0">
    <div class="catalogue__filters offcanvas offcanvas-start offcanvas-collapse w-md-50" id="offcanvasCategory"
        ...
        <div class="offcanvas-header d-lg-none mb-4">
            <p class="body__text1 fw-bold offcanvas-title text__primary mb-0" id="offcanvasCategoryLabel"><span
                    class="me-2">{{ __('Filters') }}</span><i class="far fa-sliders-up"></i></p>
            <button class="btn-close" type="button" data-bs-dismiss="offcanvas" aria-label="Close"></button>
        </div>
        <form id="filterForm" class="offcanvas-body ps-lg-2 py-0" method="GET" action="{{ route('house.index') }}">
            <div class="input-group flex-column mb-8">
                <!-- title-->
                <p class="body__text1 fw-bold text__primary d-none d-lg-block mb-3">{{ __('Search filter') }}</p>
                <div class="col-12">
                ...
                <div class="input-group flex-column my-4">
                    ...
                    <div>
                        <div class="row d-inline-flex justify-content-between align-items-center g-0"
                             id="priceRange-value">
                            <div class="col-5 d-inline-flex align-items-center justify-content-start me-auto">
                                <label class="me-3">{{ __('from') }}</label>
                                <input class="form-control-plaintext min" name="price_min" type="text"
                                       value="{{ request('price_min', $minPrice) }}"/>
                            </div>
                            <div class="col-5 d-inline-flex align-items-center justify-content-end ms-auto">
                                <label class="me-3">{{ __('before') }}</label>
                                <input class="form-control-plaintext max" name="price_max" type="text"
                                       value="{{ request('price_max', $maxPrice) }}"/>
                            </div>
                        </div>
                        <!-- range-->
                        <div class="mt-4 mb-3" id="priceRange"></div>
                    </div>
                </div>
                <!-- nav-->
